...
import pe.juabsa.cipca.foprom.api.ApiSuccessResponse
import pe.juabsa.cipca.foprom.api.FopromService
import pe.juabsa.cipca.foprom.api.RepoSearchResponse
import pe.juabsa.cipca.foprom.db.FopromDb
import pe.juabsa.cipca.foprom.db.RepoDao
import pe.juabsa.cipca.foprom.testing.OpenForTesting
import pe.juabsa.cipca.foprom.util.AbsentLiveData
import pe.juabsa.cipca.foprom.util.RateLimiter
import pe.juabsa.cipca.foprom.vo.Contributor
import pe.juabsa.cipca.foprom.vo.Repo
import pe.juabsa.cipca.foprom.vo.RepoSearchResult
...
class RepoRepository @Inject constructor(
    ...
    fun loadRepos(owner: String): LiveData<Resource<List<Repo>>> {
        return object : NetworkBoundResource<List<Repo>, List<Repo>>(appExecutors) {
            ...
            override fun createCall() = fopromService.getRepos(owner)
    ...
    fun loadRepo(owner: String, name: String): LiveData<Resource<Repo>> {
        return object : NetworkBoundResource<Repo, Repo>(appExecutors) {
            ...
            override fun createCall() = fopromService.getRepo(
                owner = owner,
                name = name
            )
        }.asLiveData()
    ...
    fun loadContributors(owner: String, name: String): LiveData<Resource<List<Contributor>>> {
        return object : NetworkBoundResource<List<Contributor>, List<Contributor>>(appExecutors) {
            ...
            override fun createCall() = fopromService.getContributors(owner, name)
        }.asLiveData()
    }
    ...
    fun search(query: String): LiveData<Resource<List<Repo>>> {
        return object : NetworkBoundResource<List<Repo>, RepoSearchResponse>(appExecutors) {
            ...
            override fun createCall() = fopromService.searchRepos(query)
